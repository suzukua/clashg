...
# 反馈问题链接
open_issue="请将安装过程日志内容(上面的所有内容)复制好后反馈给开发者,以便于帮您找到安装失败原因！反馈地址: https://github.com/zhudan/clashg/issues"
...
copy_files() {
    ...
    chmod +x /koolshare/scripts/${app_name}_*.sh

    cp -f ./uninstall.sh /koolshare/scripts/uninstall_${app_name}.sh
    chmod +x /koolshare/scripts/uninstall_${app_name}.sh
...
init_env() {
    ...
    dbus set softcenter_module_${app_name}_title="ClashG科学上网"
    dbus set softcenter_module_${app_name}_description="ClashG科学上网 for Koolshare"
